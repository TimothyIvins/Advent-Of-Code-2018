const inputData = `[1518-10-14 00:05] falls asleep
...
const getGuardID = (inString) => (inString.match(/#\d+/)[0].replace('#', ''));

const getMinute = (inString) => (inString.match(/:\d+/)[0].replace(':', ''));

const arrTimes = inputData.split("\n").sort();
...
arrTimes.forEach(inString => {
  if (inString.indexOf('Guard') > -1) {
    guardID = getGuardID(inString);
  } else if (inString.indexOf('falls asleep') > -1) {
    startMinute = getMinute(inString);
  } else if (inString.indexOf('wakes up') > -1) {
    endMinute = getMinute(inString);
    if (objMinutes[guardID]) {
      objMinutes[guardID] += (endMinute - startMinute);
    } else {
      objMinutes[guardID] = (endMinute - startMinute);
      objHour[guardID] = Array.from(Array(60), () => 0);
    };
...
Object.entries(objMinutes).forEach(([guard, minutes]) => {
...
let maxMinuteIndex = 0;
let maxMinuteValue = 0;

objHour[chosenGuard].forEach((value, index) => {
  if (value > maxMinuteValue) {
    maxMinutesIndex = index;
    maxMinuteValue = value;
  };
});

console.log("Part 1", chosenGuard * maxMinutesIndex); 
...
Object.entries(objHour).forEach(([guard, hour]) => {
  hour.forEach((value, index) => {
    if (value > maxMinuteValue) {
      maxMinutesIndex = index;
      maxMinuteValue = value;
      chosenGuard = guard;
...
console.log("Part 2", chosenGuard * maxMinutesIndex); 
